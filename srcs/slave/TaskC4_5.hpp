...
class C4_5
{
    ...
    public:
        ...
        void    comp_condnsplit_entropies(ContTable const& matrix,
                                          ...
                                          long double* cond_e = nullptr,
                                          long double* split_e = nullptr) const;
        void    comp_matrix_entropies(ContTable const& matrix,
                                      size_t total_instances,
                                      long double& cond_e,
                                      long double& split_e) const;

        long double compute_weighted_entropy(arma::Row<unsigned int> const& row,
                                             unsigned int total_row,
                                             size_t total_instances) const;
        long double compute_split_entropy(unsigned int total_row,
                                          size_t total_instances) const;
